// Inherited from https://github.com/Pquah/Beowulf/blob/master/Assets/SimpleDragAndDrop/Scripts/DragAndDropCell.cs
...
public class DragAndDropCell : MonoBehaviour, IDropHandler
{
	...
	public CellType cellType = CellType.UnlimitedSource;                               // Special type of this cell
    ...
    void Start()
    {
		...
		//DragAndDropItem myItem = GetComponentInChildren<DragAndDropItem>();
		//Debug.Log("qwe : " + myItem);
    }
    ...
    private void OnAnyItemDragStart(DragAndDropItem item)
    {
		...
		//-----------------------------Debug.Log("BIGBUG-OnAnyItemDragStart-Begin");
		if (myItem != null) // To not check empty ones
        {
            ...
            if (myItem == item)                                             // If item dragged from this cell
            {
                ...
                switch (cellType)
                {
					...
					DragAndDropItem.icon.SetActive(false);              // Item will not be dropped (In this case, dragged.)
                        break;
		...
		//-----------------------------Debug.Log("BIGBUG-OnAnyItemDragStart-End");
    }
    ...
    /// </summary>
    /// <param name="item"> dragged item </param>
    public void OnAnyItemDragEnd(DragAndDropItem item)
    {
		//-----------------------------Debug.Log("BIGBUG-OnAnyItemDragEnd-Begin");
        DragAndDropItem myItem = GetComponentInChildren<DragAndDropItem>(); // Get item from current cell
        if (myItem != null)
        {
            if (myItem == item)
		...
		//-----------------------------Debug.Log("BIGBUG-OnAnyItemDragEnd-End");
    }
                ...
                if ((item != null) && (sourceCell != this))
                {
                    ...
                    switch (cellType)                                       // Check this cell's type
                    {
                        ...
                        case CellType.DropOnly:
                            ...
                            StartCoroutine(NotifyOnDragEnd(desc));
                            break;
					case CellType.UnlimitedSource: // Ahmet Gungor, modified according to game usage.
						if(isItAColorSource == false)
						{
							PlaceItem(item.gameObject);                     // Place dropped item in this cell
							// Fill event descriptor
							desc.item = item;
